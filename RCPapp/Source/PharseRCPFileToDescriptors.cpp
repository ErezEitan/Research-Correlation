    ...
    do
    {
        ...
        else if (pos > 0)
        {
            ...
            if (eRCP_Descriptor_lineDescriptor == in_lineIndex)
            {
                ...
                {
                    m_rcpHeaderDescriptor.m_numOfBars = std::atoi(sSubType.c_str());
                }
                else if (eNumOfHistogram == numInColumn)
                {
                    m_rcpHeaderDescriptor.m_numOfHistogram = std::atoi(sSubType.c_str());
                }
            }
            else if (eHistogramDescriptor == in_lineIndex)
            {
                m_rcpHeaderDescriptor.m_vBorderLineHistogram.push_back(std::atoi(sSubType.c_str()));
            }
...
//================================================================================================================
{
    ...
    do
    {
        ...
        else if (pos > 0)
        {
            ...
            else if ((eHistogramWeight * m_rcpHeaderDescriptor.m_numOfHistogram) >= numInColumn)
            {
                m_vBarDescriptor[in_lineIndex]->m_vHistogramWeight.push_back(std::atoi(sSubType.c_str()));
            }
            else if ((eHistogramWeight * m_rcpHeaderDescriptor.m_numOfHistogram) + 1 < numInColumn)
            {
...
//==================================================
{
    ...
    m_rcpHeaderDescriptor = RCPHeaderDescriptor();
}
